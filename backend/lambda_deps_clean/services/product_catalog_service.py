...
class ProductCatalogService:
    ...
    async def find_existing_product(self, barcode=None, name=None, brand=None, image_hash=None):
        """
        Find existing product using multiple strategies (in priority order)
        
        1. Barcode exact match (barcode-index GSI)
        2. Image hash match (image_hash-index GSI)
        3. Name + Brand match (name-brand-index GSI) ← NEW - eliminates expensive scan
        """
        try:
            # Strategy 1: Barcode (fastest, most reliable)
            if barcode:
            ...
            # Strategy 2: Image Hash (good for visual duplicates)
            if image_hash:
                response = self.global_table.query(
            ...
            # Strategy 3: Name + Brand (NEW - eliminates expensive scan)
            if name and brand:
                name_brand_key = self.compute_name_brand_key(brand, name)
                response = self.global_table.query(
                    IndexName='name-brand-index',
                    KeyConditionExpression='name_brand_key = :key',
                    ExpressionAttributeValues={':key': name_brand_key}
                )
                if response['Items']:
                    return response['Items'][0]
    ...
    def compute_name_brand_key(self, brand: str, name: str) -> str:
        """
        Generate normalized key for name+brand GSI
        ...
        Returns:
            Normalized key like "nestle#maggi2minutenoodles"
        """
        import re
    ...
    async def create_global_product(self, product_data) -> str:
        ...
        try:
            ...
            # Add name_brand_key for GSI (if name and brand exist)
            if global_product.name and global_product.brand:
                item_dict['name_brand_key'] = self.compute_name_brand_key(
                    global_product.brand, 
                    global_product.name
                )
