...
@app.post("/api/v1/customer/auth/google")
async def customer_google_auth(request: Request):
    ...
    try:
        ...
        try:
            ...
            if items:
                ...
                customer_id = items[0].get('id', {}).get('S', '')
                dynamodb.update_item(
                    TableName='vyaparai-users-prod',
                    Key={'id': {'S': customer_id}},
                    UpdateExpression='SET #name = :name, google_id = :google_id, picture = :picture, last_login = :last_login, auth_provider = :auth_provider',
                    ExpressionAttributeNames={'#name': 'name'},
                    ExpressionAttributeValues={
                        ':name': {'S': name or email.split('@')[0]},
                        ':google_id': {'S': google_id},
                        ':picture': {'S': picture or ''},
                        ':last_login': {'S': datetime.utcnow().isoformat()},
                ...
                dynamodb.put_item(
                    ...
                    Item={
                        ...
                        'created_at': {'S': datetime.utcnow().isoformat()},
                        'last_login': {'S': datetime.utcnow().isoformat()},
                        'verified': {'BOOL': True}  # Google accounts are pre-verified
                    }
                )

            # Create JWT token
            token = create_jwt_token(customer_id, email, '', 'customer')
...
@app.post("/api/v1/customer/auth/facebook")
async def customer_facebook_auth(request: Request):
    ...
    try:
        ...
        try:
            # First, verify the access token with Facebook's debug_token endpoint
            if app_secret:
                # Get app access token (app_id|app_secret)
                app_id = '788136330687220'
                app_access_token = f'{app_id}|{app_secret}'

                # Verify the user's access token
                verify_response = requests.get(
                    f'https://graph.facebook.com/debug_token?input_token={access_token}&access_token={app_access_token}'
                )
                ...
                if verify_response.status_code == 200:
                    ...
                    if token_data.get('app_id') != app_id:
                        raise HTTPException(status_code=400, detail="Token does not belong to this app")

                    logger.info(f"Facebook token verified successfully for user {token_data.get('user_id')}")
                else:
                    logger.warning("Could not verify Facebook token with app secret")
            ...
            if items:
                ...
                dynamodb.update_item(
                    TableName='vyaparai-users-prod',
                    Key={'id': {'S': customer_id}},
                    UpdateExpression='SET #name = :name, facebook_id = :facebook_id, picture = :picture, last_login = :last_login, auth_provider = :auth_provider',
                    ExpressionAttributeNames={'#name': 'name'},
                    ExpressionAttributeValues={
                        ':name': {'S': name or email.split('@')[0]},
                        ':facebook_id': {'S': facebook_id},
                        ':picture': {'S': picture},
                        ':last_login': {'S': datetime.utcnow().isoformat()},
                        ':auth_provider': {'S': 'facebook'}
                    }
                )
            else:
                # New customer - create account
                ...
                dynamodb.put_item(
                    ...
                    Item={
                        ...
                        'created_at': {'S': datetime.utcnow().isoformat()},
                        'last_login': {'S': datetime.utcnow().isoformat()},
                        'verified': {'BOOL': True}  # Facebook accounts are pre-verified
                    }
                )
