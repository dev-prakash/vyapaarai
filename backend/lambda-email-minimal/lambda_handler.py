...
async def get_store_by_email(email):
    """Get store details from DynamoDB by email using email-index GSI"""
    # Get DynamoDB client
    ...
    try:
        # Use Query with email-index GSI instead of Scan (90% cost reduction)
...
async def get_inventory_products(
    ...
    """Get inventory products for a store with pagination"""
    try:
        # If store_id not provided as query param, extract from JWT token
        if not store_id:
            auth_header = request.headers.get('Authorization')
        ...
        # Query inventory items with pagination and projection
        query_params = {
            'TableName': TABLE_NAMES['inventory'],
            'KeyConditionExpression': 'store_id = :store_id',
            'ExpressionAttributeValues': {
            ...
            'Limit': min(limit, 500),  # Max 500 items per page
            'ProjectionExpression': 'inventory_id, product_id, product_name, current_stock, min_stock_level, max_stock_level, reorder_point, cost_price, selling_price, mrp, discount_percentage, #loc, is_active, created_at, updated_at, description',
        ...
        if last_key:
            try:
                import base64
                import json
                query_params['ExclusiveStartKey'] = json.loads(base64.b64decode(last_key))
            except Exception as e:
                logger.warning(f"Invalid last_key: {e}")

        response = local_dynamodb.query(**query_params)

        items = response.get('Items', [])

        # Collect all product IDs for batch fetch (fixes N+1 problem)
        product_ids = [item.get('product_id', {}).get('S') for item in items if item.get('product_id', {}).get('S')]

        # Batch fetch product details from global catalog (100 items per batch)
        product_details = {}
        for i in range(0, len(product_ids), 100):
            chunk = product_ids[i:i+100]
            try:
                batch_response = local_dynamodb.batch_get_item(
                    RequestItems={
                        TABLE_NAMES['products']: {
                            'Keys': [{'product_id': {'S': pid}} for pid in chunk],
                            'ProjectionExpression': 'product_id, #n, brand, category, barcode, image_url, description, mrp',
                            'ExpressionAttributeNames': {'#n': 'name'}
                        }
                    }
                )
                for prod_item in batch_response.get('Responses', {}).get(TABLE_NAMES['products'], []):
                    pid = prod_item.get('product_id', {}).get('S')
                    if pid:
                        product_details[pid] = prod_item
            except Exception as e:
                logger.warning(f"Batch fetch error for chunk {i}: {e}")

        # Build products list with enriched data
        products = []
        for item in items:
            try:
                product_id = item.get('product_id', {}).get('S', '')
                ...
                # Get additional product details from batch-fetched data
                category = ''
                brand = ''
                barcode = ''
                image_url = ''
                ...
                if product_id and product_id in product_details:
                    prod_item = product_details[product_id]
...
async def register_customer(request: dict):
    ...
    try:
        ...
        # Validate phone number format (Indian: +91XXXXXXXXXX)
        ...
        if 'email' in request and request['email']:
            customer_item['email'] = {'S': request['email']}
            customer_item['email_verified'] = {'BOOL': False}
...
@app.post("/api/v1/customers/send-otp")
async def send_customer_otp(request: dict):
    ...
    try:
        ...
        # Store OTP in passcodes table (reuse existing table)
        local_dynamodb = get_dynamodb_client()
        if not local_dynamodb:
            raise HTTPException(status_code=500, detail="Database unavailable")

        ttl = int(time.time()) + (15 * 60)  # 15 minutes
...
@app.post("/api/v1/customers/verify-otp")
async def verify_customer_otp(request: dict):
    ...
    try:
        ...
        try:
            response = local_dynamodb.get_item(
                TableName=TABLE_NAMES['passcodes'],
                Key={'identifier': {'S': phone}}
            )

            if 'Item' not in response:
                raise HTTPException(status_code=401, detail="Invalid or expired OTP")

            stored_otp = response['Item'].get('passcode', {}).get('S')
            if stored_otp != otp:
        ...
        customer_response = local_dynamodb.query(
            TableName=TABLE_NAMES['customers'],
            IndexName='phone-index',
            KeyConditionExpression='phone = :phone',
            ExpressionAttributeValues={
        ...
        if not customer_response.get('Items'):
            raise HTTPException(status_code=404, detail="Customer not found. Please register first.")

        customer = customer_response['Items'][0]
        customer_id = customer.get('customer_id', {}).get('S')

        # Update last_login_at
        local_dynamodb.update_item(
            TableName=TABLE_NAMES['customers'],
            Key={'customer_id': {'S': customer_id}},
            UpdateExpression='SET last_login_at = :now, phone_verified = :verified',
            ExpressionAttributeValues={
...
@app.get("/api/v1/customers/profile")
async def get_customer_profile(request: Request):
    ...
    try:
        ...
        # Parse addresses and payment methods
        addresses = []
        for addr in customer.get('addresses', {}).get('L', []):
            addr_map = addr.get('M', {})
            addresses.append({
                'address_id': addr_map.get('address_id', {}).get('S'),
                'is_default': addr_map.get('is_default', {}).get('BOOL', False),
                'type': addr_map.get('type', {}).get('S'),
                'recipient_name': addr_map.get('recipient_name', {}).get('S'),
                'line1': addr_map.get('line1', {}).get('S'),
                'line2': addr_map.get('line2', {}).get('S'),
                'city': addr_map.get('city', {}).get('S'),
                'state': addr_map.get('state', {}).get('S'),
                'pincode': addr_map.get('pincode', {}).get('S'),
                'phone': addr_map.get('phone', {}).get('S'),
                'landmark': addr_map.get('landmark', {}).get('S')
            })

        payment_methods = []
        for pm in customer.get('payment_methods', {}).get('L', []):
            pm_map = pm.get('M', {})
            payment_methods.append({
                'payment_id': pm_map.get('payment_id', {}).get('S'),
                'type': pm_map.get('type', {}).get('S'),
                'is_default': pm_map.get('is_default', {}).get('BOOL', False),
                'upi_id': pm_map.get('upi_id', {}).get('S'),
                'last4': pm_map.get('last4', {}).get('S'),
                'network': pm_map.get('network', {}).get('S')
            })
        ...
        return {
            ...
            "customer": {
                ...
                "default_address_id": customer.get('default_address_id', {}).get('S'),
                "default_payment_id": customer.get('default_payment_id', {}).get('S'),
                "order_count": int(customer.get('order_count', {}).get('N', '0')),
                "total_spent": float(customer.get('total_spent', {}).get('N', '0')),
                "status": customer.get('status', {}).get('S'),
                "created_at": customer.get('created_at', {}).get('S')
            }
        }
...
@app.post("/api/v1/customers/addresses")
async def add_customer_address(request: Request, address_data: dict):
    ...
    try:
        ...
        # Validate required fields
        required_fields = ['line1', 'city', 'state', 'pincode', 'phone']
        for field in required_fields:
        ...
        new_address = {
            'M': {
                ...
                'type': {'S': address_data.get('type', 'home')},
                'recipient_name': {'S': address_data.get('recipient_name', '')},
                'line1': {'S': address_data['line1']},
                'line2': {'S': address_data.get('line2', '')},
                'city': {'S': address_data['city']},
                'state': {'S': address_data['state']},
                'pincode': {'S': address_data['pincode']},
                'phone': {'S': address_data['phone']},
                'landmark': {'S': address_data.get('landmark', '')},
                'created_at': {'S': datetime.utcnow().isoformat()}
            }
        }

        # If this is default address, unset other defaults first
        if address_data.get('is_default', False):
            # Get current customer
            customer_resp = local_dynamodb.get_item(
                TableName=TABLE_NAMES['customers'],
                Key={'customer_id': {'S': customer_id}}
            )
            
            if 'Item' in customer_resp:
                addresses = customer_resp['Item'].get('addresses', {}).get('L', [])
                for addr in addresses:
                    if addr.get('M', {}).get('is_default', {}).get('BOOL'):
                        addr['M']['is_default'] = {'BOOL': False}
        ...
        local_dynamodb.update_item(
            TableName=TABLE_NAMES['customers'],
            Key={'customer_id': {'S': customer_id}},
            UpdateExpression='SET addresses = list_append(if_not_exists(addresses, :empty_list), :new_address), default_address_id = if_not_exists(default_address_id, :addr_id), updated_at = :now',
            ExpressionAttributeValues={
                ':new_address': {'L': [new_address]},
                ':empty_list': {'L': []},
                ':addr_id': {'S': address_id},
                ':now': {'S': datetime.utcnow().isoformat()}
            }
        )
...
@app.put("/api/v1/customers/addresses/{address_id}/set-default")
async def set_default_address(request: Request, address_id: str):
    ...
    try:
        ...
        for addr in addresses:
            addr_map = addr.get('M', {})
            if addr_map.get('address_id', {}).get('S') == address_id:
                addr_map['is_default'] = {'BOOL': True}
                address_found = True
            else:
                addr_map['is_default'] = {'BOOL': False}
        ...
        local_dynamodb.update_item(
            TableName=TABLE_NAMES['customers'],
            Key={'customer_id': {'S': customer_id}},
            UpdateExpression='SET addresses = :addresses, default_address_id = :addr_id, updated_at = :now',
            ExpressionAttributeValues={
                ':addresses': {'L': addresses},
                ':addr_id': {'S': address_id},
                ':now': {'S': datetime.utcnow().isoformat()}
...
@app.delete("/api/v1/customers/addresses/{address_id}")
async def delete_customer_address(request: Request, address_id: str):
    ...
    try:
        ...
        addresses = customer_resp['Item'].get('addresses', {}).get('L', [])
        filtered_addresses = [addr for addr in addresses if addr.get('M', {}).get('address_id', {}).get('S') != address_id]
        ...
        local_dynamodb.update_item(
            TableName=TABLE_NAMES['customers'],
            Key={'customer_id': {'S': customer_id}},
            UpdateExpression='SET addresses = :addresses, updated_at = :now',
            ExpressionAttributeValues={
                ':addresses': {'L': filtered_addresses},
                ':now': {'S': datetime.utcnow().isoformat()}
            }
        )
...
@app.post("/api/v1/customers/payment-methods")
async def add_payment_method(request: Request, payment_data: dict):
    ...
    try:
        ...
        if 'type' not in payment_data:
            raise HTTPException(status_code=400, detail="Payment type is required")

        payment_type = payment_data['type']
        if payment_type not in ['upi', 'card', 'netbanking', 'cod', 'wallet']:
        ...
        # Generate payment ID
        payment_id = f"pm_{uuid.uuid4().hex[:12]}"
        ...
        new_payment = {
            'M': {
                ...
                'type': {'S': payment_type},
                'is_default': {'BOOL': payment_data.get('is_default', False)},
                'created_at': {'S': datetime.utcnow().isoformat()}
            }
        }
        ...
        if payment_type == 'upi':
            if 'upi_id' not in payment_data:
                raise HTTPException(status_code=400, detail="UPI ID is required")
            new_payment['M']['upi_id'] = {'S': payment_data['upi_id']}
            new_payment['M']['provider'] = {'S': payment_data.get('provider', 'unknown')}

        elif payment_type == 'card':
            if 'token' not in payment_data or 'last4' not in payment_data:
                raise HTTPException(status_code=400, detail="Card token and last4 digits required")
            new_payment['M']['token'] = {'S': payment_data['token']}
            new_payment['M']['last4'] = {'S': payment_data['last4']}
            new_payment['M']['network'] = {'S': payment_data.get('network', 'unknown')}
            new_payment['M']['expiry_month'] = {'S': payment_data.get('expiry_month', '')}
            new_payment['M']['expiry_year'] = {'S': payment_data.get('expiry_year', '')}

        # If this is default payment, unset other defaults first
        if payment_data.get('is_default', False):
            ...
            if 'Item' in customer_resp:
                payment_methods = customer_resp['Item'].get('payment_methods', {}).get('L', [])
                for pm in payment_methods:
                    if pm.get('M', {}).get('is_default', {}).get('BOOL'):
                        pm['M']['is_default'] = {'BOOL': False}
                
                local_dynamodb.update_item(
                    TableName=TABLE_NAMES['customers'],
                    Key={'customer_id': {'S': customer_id}},
                    UpdateExpression='SET payment_methods = :payment_methods',
                    ExpressionAttributeValues={
        ...
        local_dynamodb.update_item(
            TableName=TABLE_NAMES['customers'],
            Key={'customer_id': {'S': customer_id}},
            UpdateExpression='SET payment_methods = list_append(if_not_exists(payment_methods, :empty_list), :new_payment), default_payment_id = if_not_exists(default_payment_id, :pm_id), updated_at = :now',
            ExpressionAttributeValues={
                ':new_payment': {'L': [new_payment]},
                ':empty_list': {'L': []},
                ':pm_id': {'S': payment_id},
                ':now': {'S': datetime.utcnow().isoformat()}
            }
...
@app.delete("/api/v1/customers/payment-methods/{payment_id}")
async def delete_payment_method(request: Request, payment_id: str):
    ...
    try:
        ...
        payment_methods = customer_resp['Item'].get('payment_methods', {}).get('L', [])
        filtered_methods = [pm for pm in payment_methods if pm.get('M', {}).get('payment_id', {}).get('S') != payment_id]

        if len(filtered_methods) == len(payment_methods):
        ...
        local_dynamodb.update_item(
            TableName=TABLE_NAMES['customers'],
            Key={'customer_id': {'S': customer_id}},
            UpdateExpression='SET payment_methods = :payment_methods, updated_at = :now',
            ExpressionAttributeValues={
                ':payment_methods': {'L': filtered_methods},
                ':now': {'S': datetime.utcnow().isoformat()}
            }
        )
...
# Customer Order Management APIs - Enterprise Grade
# Based on patterns from kirana-voice-enterprise

# Add these endpoints after the existing customer payment methods APIs (around line 3900)

@app.post("/api/v1/customers/orders")
async def create_customer_order(request: Request, order_data: dict):
    """
    Create order for customer with saved address and payment method
    Enterprise-grade order creation with validation, tracking, and audit trail
    """
    local_dynamodb = get_dynamodb_client()
    if not local_dynamodb:
        raise HTTPException(status_code=500, detail="Database connection failed")
    ...
    try:
        ...
        if payload.get('type') != 'customer':
            raise HTTPException(status_code=403, detail="Invalid token type. Customer token required")
    ...
    # Validate required fields
    required_fields = ['store_id', 'address_id', 'payment_method_id', 'items']
    for field in required_fields:
    ...
    for item in order_data['items']:
        # Accept both product_id (from frontend/cart) and inventory_id (legacy)
        if ('product_id' not in item and 'inventory_id' not in item) or 'quantity' not in item:
            raise HTTPException(status_code=400, detail="Each item must have product_id (or inventory_id) and quantity")
        if item['quantity'] <= 0:
            raise HTTPException(status_code=400, detail="Item quantity must be greater than 0")

    try:
        # Get customer profile
        customer_response = local_dynamodb.get_item(
            TableName=TABLE_NAMES['customers'],
            Key={'customer_id': {'S': customer_id}}
        )
        ...
        # Parse addresses and payment methods
        addresses = []
        if customer.get('addresses', {}).get('L'):
            for addr in customer['addresses']['L']:
                if addr.get('M'):
                    addresses.append({
                        'address_id': addr['M'].get('address_id', {}).get('S'),
                        'line1': addr['M'].get('line1', {}).get('S'),
                        'line2': addr['M'].get('line2', {}).get('S', ''),
                        'city': addr['M'].get('city', {}).get('S'),
                        'state': addr['M'].get('state', {}).get('S'),
                        'pincode': addr['M'].get('pincode', {}).get('S'),
                        'phone': addr['M'].get('phone', {}).get('S'),
                        'landmark': addr['M'].get('landmark', {}).get('S', ''),
                        'type': addr['M'].get('type', {}).get('S', 'home'),
                    })

        # Find selected address
        selected_address = None
        for addr in addresses:
            if addr['address_id'] == order_data['address_id']:
        ...
        payment_methods = []
        if customer.get('payment_methods', {}).get('L'):
            for pm in customer['payment_methods']['L']:
                if pm.get('M'):
                    payment_methods.append({
                        'payment_id': pm['M'].get('payment_id', {}).get('S'),
                        'type': pm['M'].get('type', {}).get('S'),
                    })

        # Find selected payment method
        selected_payment = None
        for pm in payment_methods:
            if pm['payment_id'] == order_data['payment_method_id']:
                selected_payment = pm
                break
        ...
        # Fetch inventory items and calculate total
        items_with_details = []
        items_for_stock_update = []  # Track items for stock update
        subtotal = 0

        for item in order_data['items']:
            # Use product_id (preferred) or fall back to inventory_id for legacy support
            product_id = item.get('product_id') or item.get('inventory_id')

            inv_response = local_dynamodb.get_item(
                TableName=TABLE_NAMES['inventory'],
                Key={
                    'store_id': {'S': order_data['store_id']},
                    'product_id': {'S': product_id}  # DynamoDB schema uses product_id as sort key
                }
            )
            ...
            inv_item = inv_response['Item']

            # Check stock availability
            current_stock = int(inv_item.get('current_stock', {}).get('N', 0))
            if current_stock < item['quantity']:
                raise HTTPException(
                    status_code=400,
                    detail=f"Insufficient stock for {inv_item.get('product_name', {}).get('S', 'item')}. Available: {current_stock}"
                )

            # Calculate item total
            selling_price = float(inv_item.get('selling_price', {}).get('N', 0))
            item_total = selling_price * item['quantity']
            subtotal += item_total

            # Store product_id and quantity for stock update
            ...
            items_with_details.append({
                'M': {
                    'inventory_id': {'S': inv_item.get('inventory_id', {}).get('S', '')},  # Use actual inventory_id from DB
                    'product_id': {'S': inv_item.get('product_id', {}).get('S', '')},
                    'product_name': {'S': inv_item.get('product_name', {}).get('S', '')},
                    'quantity': {'N': str(item['quantity'])},
                    'unit_price': {'N': str(selling_price)},
                    'item_total': {'N': str(item_total)},
                    'mrp': {'N': inv_item.get('mrp', {}).get('N', '0')},
                }
            })
        ...
        # Count today's orders for this store to generate sequence
        today_start = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0).isoformat()
        count_response = local_dynamodb.query(
            TableName=TABLE_NAMES['orders'],
            KeyConditionExpression='store_id = :store_id',
        ...
        order_sequence = count_response.get('Count', 0) + 1
        order_number = f"ORD-{date_str}-{str(order_sequence).zfill(5)}"
        ...
        order_item = {
            ...
            'customer_id': {'S': customer_id},
            'customer_name': {'S': f"{customer.get('first_name', {}).get('S', '')} {customer.get('last_name', {}).get('S', '')}"},
            'customer_phone': {'S': customer.get('phone', {}).get('S', '')},
            'status': {'S': 'placed'},
            'payment_status': {'S': 'pending'},
            'payment_method': {'S': selected_payment['type']},
            'items': {'L': items_with_details},
            'subtotal': {'N': str(subtotal)},
            'delivery_fee': {'N': str(delivery_fee)},
            'total': {'N': str(total)},
            'delivery_address': {'M': {
                'address_id': {'S': selected_address['address_id']},
                'line1': {'S': selected_address['line1']},
                'line2': {'S': selected_address.get('line2', '')},
                'city': {'S': selected_address['city']},
                'state': {'S': selected_address['state']},
                'pincode': {'S': selected_address['pincode']},
                'phone': {'S': selected_address['phone']},
                'landmark': {'S': selected_address.get('landmark', '')},
                'type': {'S': selected_address.get('type', 'home')},
            }},
            'customer_note': {'S': order_data.get('notes', '')},
            'delivery_instructions': {'S': order_data.get('delivery_instructions', '')},
            'source': {'S': 'customer_web'},
            'created_at': {'S': now},
            'updated_at': {'S': now},
        }

        # Add tracking_id for customer order tracking
        tracking_id = f"TRK{uuid.uuid4().hex[:12].upper()}"
        order_item['tracking_id'] = {'S': tracking_id}
        ...
        for item_detail in items_for_stock_update:
            inv_response = local_dynamodb.get_item(
                TableName=TABLE_NAMES['inventory'],
                Key={
                    'store_id': {'S': order_data['store_id']},
                    'product_id': {'S': item_detail['product_id']}  # Use product_id, not inventory_id
                }
            )

            if 'Item' in inv_response:
                current_stock = int(inv_response['Item'].get('current_stock', {}).get('N', 0))
                new_stock = current_stock - item_detail['quantity']
                ...
                local_dynamodb.update_item(
                    ...
                    UpdateExpression='SET current_stock = :new_stock, updated_at = :now',
                    ExpressionAttributeValues={
                        ':new_stock': {'N': str(new_stock)},
                        ':now': {'S': now}
                    }
        ...
        current_order_count = int(customer.get('order_count', {}).get('N', 0))
        current_total_spent = float(customer.get('total_spent', {}).get('N', 0))

        local_dynamodb.update_item(
            TableName=TABLE_NAMES['customers'],
            Key={'customer_id': {'S': customer_id}},
            UpdateExpression='SET order_count = :order_count, total_spent = :total_spent, updated_at = :now',
            ExpressionAttributeValues={
                ':order_count': {'N': str(current_order_count + 1)},
                ':total_spent': {'N': str(current_total_spent + total)},
                ':now': {'S': now}
            }
        )
        ...
        local_dynamodb.update_item(
            ...
            UpdateExpression='SET addresses = :addresses',
            ExpressionAttributeValues={
                ':addresses': customer.get('addresses', {'L': []})
            }
        )
        ...
        logger.error(f"Create customer order error: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Failed to create order: {str(e)}")


@app.get("/api/v1/customers/orders")
async def get_customer_orders(request: Request, limit: int = 20, last_key: str = None):
    ...
    try:
        # Query orders using customer-index GSI
        query_params = {
            'TableName': TABLE_NAMES['orders'],
        ...
        for item in response.get('Items', []):
            # Parse items
            items = []
            if item.get('items', {}).get('L'):
                for order_item in item['items']['L']:
                    if order_item.get('M'):
                        items.append({
                            'inventory_id': order_item['M'].get('inventory_id', {}).get('S', ''),
                            'product_name': order_item['M'].get('product_name', {}).get('S', ''),
                            'quantity': int(order_item['M'].get('quantity', {}).get('N', 0)),
                            'unit_price': float(order_item['M'].get('unit_price', {}).get('N', 0)),
                            'item_total': float(order_item['M'].get('item_total', {}).get('N', 0)),
                        })

            orders.append({
                'order_id': item.get('id', {}).get('S', ''),
                'order_number': item.get('order_number', {}).get('S', ''),
                'tracking_id': item.get('tracking_id', {}).get('S', ''),
                'status': item.get('status', {}).get('S', ''),
                'payment_status': item.get('payment_status', {}).get('S', ''),
                'payment_method': item.get('payment_method', {}).get('S', ''),
                'items': items,
                'subtotal': float(item.get('subtotal', {}).get('N', 0)),
                'delivery_fee': float(item.get('delivery_fee', {}).get('N', 0)),
...
@app.get("/api/v1/customers/orders/{order_id}")
async def get_customer_order_detail(request: Request, order_id: str):
    ...
    try:
        # Query using customer-index to ensure customer owns this order
        response = local_dynamodb.query(
            TableName=TABLE_NAMES['orders'],
            IndexName='customer-index',
            KeyConditionExpression='customer_id = :customer_id',
            FilterExpression='id = :order_id',
            ExpressionAttributeValues={
                ':customer_id': {'S': customer_id},
                ':order_id': {'S': order_id}
            },
        ...
        delivery_address = {}
        if item.get('delivery_address', {}).get('M'):
            addr = item['delivery_address']['M']
            delivery_address = {
                'line1': addr.get('line1', {}).get('S', ''),
                'line2': addr.get('line2', {}).get('S', ''),
                'city': addr.get('city', {}).get('S', ''),
                'state': addr.get('state', {}).get('S', ''),
                'pincode': addr.get('pincode', {}).get('S', ''),
                'phone': addr.get('phone', {}).get('S', ''),
                'landmark': addr.get('landmark', {}).get('S', ''),
            }
        ...
        order_detail = {
            ...
            'customer_note': item.get('customer_note', {}).get('S', ''),
            'delivery_instructions': item.get('delivery_instructions', {}).get('S', ''),
            'created_at': item.get('created_at', {}).get('S', ''),
            'updated_at': item.get('updated_at', {}).get('S', ''),
        }
...
@app.post("/api/v1/customers/orders/{order_id}/cancel")
async def cancel_customer_order(request: Request, order_id: str, cancel_data: dict):
    ...
    try:
        ...
        current_status = order.get('status', {}).get('S', '')

        # Check if cancellation is allowed
        if current_status not in ['placed', 'confirmed']:
            raise HTTPException(
                status_code=400,
                detail=f"Cannot cancel order with status '{current_status}'. Only 'placed' or 'confirmed' orders can be cancelled."
            )

        # Update order status to cancelled
        now = datetime.utcnow().isoformat()
        cancel_reason = cancel_data.get('reason', 'Cancelled by customer')

        local_dynamodb.update_item(
            TableName=TABLE_NAMES['orders'],
            Key={
                'store_id': {'S': order.get('store_id', {}).get('S', '')},
                'id': {'S': order_id}
            },
            UpdateExpression='SET #status = :status, updated_at = :now, cancel_reason = :reason',
            ExpressionAttributeNames={
                '#status': 'status'
            },
            ExpressionAttributeValues={
        ...
        # Restore inventory
        if order.get('items', {}).get('L'):
            for item in order['items']['L']:
                if item.get('M'):
                    inventory_id = item['M'].get('inventory_id', {}).get('S')
                    quantity = int(item['M'].get('quantity', {}).get('N', 0))
