...
# No need for CORS middleware in the Lambda function
...
TABLE_NAMES = {
    ...
    'products': 'vyaparai-store-inventory-prod',
...
async def get_products_by_store(store_id: str) -> List[Dict]:
    ...
    try:
        response = dynamodb.scan(
            TableName=TABLE_NAMES['products'],
            FilterExpression='store_id = :store_id AND is_active = :active',
            ExpressionAttributeValues={
                ':store_id': {'S': store_id},
                ':active': {'BOOL': True}
            }
        ...
        for item in response.get('Items', []):
            try:
                # Map actual DynamoDB fields from vyaparai-store-inventory-prod table
                product = {
                    'id': item.get('product_id', {}).get('S', ''),
                    'store_id': item.get('store_id', {}).get('S', ''),
                    'name': item.get('notes', {}).get('S', 'Product'),
                    'description': item.get('description', {}).get('S', ''),
                    'category': item.get('category', {}).get('S', 'general'),
                    'price': float(item.get('selling_price', {}).get('N', 0)),
                    'cost_price': float(item.get('cost_price', {}).get('N', 0)),
                    'mrp': float(item.get('mrp', {}).get('N', 0)),
                    'stock_quantity': int(item.get('current_stock', {}).get('N', 0)),
                    ...
                    'status': 'active' if item.get('is_active', {}).get('BOOL', True) else 'inactive',
    ...
    try:
        ...
        for item in result['items']:
            ...
            product = {
                ...
                "quantity": inventory_data.get('current_stock', 0),  # Map from actual DB field
                "stock_quantity": inventory_data.get('current_stock', 0),  # Frontend expects this field
                "current_stock": inventory_data.get('current_stock', 0),  # Alias for compatibility
                ...
                "reorder_level": inventory_data.get('reorder_point', inventory_data.get('min_stock_level', 10)),
                "min_stock_level": inventory_data.get('min_stock_level', inventory_data.get('reorder_point', 10)),  # Frontend expects this
                "max_stock_level": inventory_data.get('max_stock_level', 100),  # Use actual value from DB
...
async def get_inventory_summary(request: Request):
    """Get inventory summary statistics - lightweight endpoint for dashboard"""
    try:
        # Extract store_id from JWT
        store_id = get_store_from_jwt(request)
        ...
        # Query store inventory with minimal projection (only fields needed for calculation)
        response = dynamodb.query(
            ...
            ProjectionExpression='current_stock, selling_price, min_stock_level, is_active'
        )
...
async def search_by_barcode(request: Request, barcode: str):
    ...
    try:
        ...
        logger.info(f"[BarcodeSearch] Store: {store_id}, Barcode: {barcode}")

        # Search for existing product in store inventory
        store_product = await get_product_by_barcode(store_id, barcode)

        if store_product:
            logger.info(f"[BarcodeAnalytics] Result: FOUND_IN_STORE | Barcode: {barcode} | Store: {store_id}")
            return {
                "success": True,
        ...
        # Search for generic product in global catalog
        generic_product = await get_generic_product_by_barcode(barcode)

        if generic_product:
            logger.info(f"[BarcodeAnalytics] Result: FOUND_IN_CATALOG | Barcode: {barcode} | Product: {generic_product.get('name', 'Unknown')} | Store: {store_id}")
            return {
                "success": True,
        ...
        logger.info(f"[BarcodeAnalytics] Result: NOT_FOUND | Barcode: {barcode} | Store: {store_id}")
        ...
        logger.error(f"[BarcodeAnalytics] Result: ERROR | Barcode: {barcode} | Error: {str(e)}")
...
async def browse_global_catalog(request: Request, search: str = '', category: str = '', limit: int = 50):
    """Browse active global product catalog for store owners"""
    try:
        ...
        # Helper function to convert DynamoDB Map to Python dict
        def dynamodb_map_to_dict(dynamo_map):
            result = {}
            for key, value in dynamo_map.items():
                if 'S' in value:
                    ...
                    result[key] = dynamodb_map_to_dict(value['M'])
            return result
        ...
        for item in response.get('Items', []):
            product = {
                ...
                'attributes': dynamodb_map_to_dict(item.get('attributes', {}).get('M', {})),
            ...
            if search:
                ...
                if (search_lower not in product['name'].lower() and
                    search_lower not in product['brand'].lower() and
                    search_lower not in product.get('barcode', '')):
                    continue
        ...
        logger.error(f"Browse global catalog error: {e}")
        return {'success': False, 'error': str(e)}

@app.post("/api/v1/inventory/products/from-catalog")
async def add_from_global_catalog(request: Request):
    """Add a product from global catalog to store inventory"""
    try:
        # Get store_id from JWT
        store_id = get_store_from_jwt(request)
        if not store_id:
            return {'success': False, 'error': 'Authentication required'}

        body = await request.json()
        global_product_id = body.get('global_product_id')
        if not global_product_id:
            return {'success': False, 'error': 'global_product_id is required'}

        dynamodb = get_dynamodb_client()
        if not dynamodb:
            return {'success': False, 'error': 'Database connection failed'}

        # 1. Verify global product exists and is active
        global_response = dynamodb.get_item(
            TableName='vyaparai-global-products-prod',
            Key={'product_id': {'S': global_product_id}}
        )

        if 'Item' not in global_response:
            return {'success': False, 'error': 'Global product not found'}

        global_item = global_response['Item']
        if global_item.get('status', {}).get('S') != 'active':
            return {'success': False, 'error': 'Product is not active in catalog'}

        # 2. Check if already exists in store inventory
        inventory_check = dynamodb.get_item(
        ...
        if 'Item' in inventory_check:
            return {'success': False, 'error': 'Product already exists in your inventory'}
        ...
        inventory_item = {
            'store_id': {'S': store_id},
            'product_id': {'S': global_product_id},
            'global_product_id': {'S': global_product_id},  # Reference to global catalog
            'current_stock': {'N': str(body.get('current_stock', 0))},
            'selling_price': {'N': str(body.get('selling_price', 0))},
            'cost_price': {'N': str(body.get('cost_price', 0))},
            'min_stock_level': {'N': str(body.get('min_stock_level', 5))},
            'max_stock_level': {'N': str(body.get('max_stock_level', 100))},
            'reorder_point': {'N': str(body.get('reorder_point', 10))},
            'is_active': {'BOOL': body.get('is_active', True)},
            'location': {'S': body.get('location', '')},
        ...
        # 4. Increment stores_using_count in global product
        try:
            dynamodb.update_item(
                TableName='vyaparai-global-products-prod',
                Key={'product_id': {'S': global_product_id}},
            ...
            logger.warning(f"Failed to increment stores_using_count: {e}")

        logger.info(f"Added global product {global_product_id} to store {store_id}")

        # Return complete product details for frontend
        image_urls = global_item.get('canonical_image_urls', {})
        if not image_urls and global_item.get('image', {}).get('S'):
            legacy_image = global_item['image']['S']
            image_urls = {
                'original': legacy_image,
                'thumbnail': legacy_image,
                'medium': legacy_image,
        ...
        product_response = {
            ...
            "product_id": global_product_id,
            "global_product_id": global_product_id,
            "name": global_item.get('name', {}).get('S', ''),
            "brand": global_item.get('brand', {}).get('S'),
            "category": global_item.get('category', {}).get('S'),
            "barcode": global_item.get('barcode', {}).get('S'),
            "canonical_image_urls": image_urls,
            "image": image_urls.get('original') if image_urls else None,

            # Store-specific fields from request
            "inventory_id": f"{store_id}_{global_product_id}",
            "quantity": body.get('current_stock', 0),
            "stock_quantity": body.get('current_stock', 0),
            "current_stock": body.get('current_stock', 0),
            "cost_price": body.get('cost_price', 0),
            "selling_price": body.get('selling_price', 0),
            "price": body.get('selling_price', 0),
            "min_stock_level": body.get('min_stock_level', 5),
            "max_stock_level": body.get('max_stock_level', 100),
            "reorder_point": body.get('reorder_point', 10),
            "location": body.get('location', ''),
            "notes": body.get('notes', ''),
            "status": "active",
            "is_shared_product": True,
        ...
        logger.error(f"Add from catalog error: {e}")
        return {'success': False, 'error': str(e)}
...
async def get_generic_product_by_barcode(barcode: str):
    """Get generic product information by barcode from global catalog"""
    try:
        dynamodb = get_dynamodb_client()
        if not dynamodb:
            logger.error("Failed to get DynamoDB client")
            return None

        # Query global products table using barcode-index GSI
        response = dynamodb.query(
            TableName='vyaparai-global-products-prod',
        ...
        if items:
            ...
            def get_num(field, default=0):
                val = item.get(field, {}).get('N', str(default))
                return int(val) if val else default

            def get_map(field):
                m = item.get(field, {}).get('M', {})
                # Convert DynamoDB Map format to simple dict
                result = {}
            ...
            # Parse and return rich product data
            generic_product = {
                ...
                'canonical_image_urls': get_map('canonical_image_urls'),
                'verification_status': get_str('verification_status', 'verified'),
                'attributes': get_map('attributes'),
                'regional_names': get_map('regional_names'),
                'created_at': get_str('created_at'),
                'stores_using_count': get_num('stores_using_count', 0)
            }

            logger.info(f"✅ Found product in global catalog: {generic_product['name']} ({barcode})")
            return generic_product

        # Not found in database
        logger.info(f"❌ Barcode {barcode} not found in global catalog")
        return None
    ...
    try:
        ...
        if 'Item' in response:
            ...
            return {
                'id': item.get('product_id', {}).get('S', ''),
                'product_id': item.get('product_id', {}).get('S', ''),
                'product_name': item.get('notes', {}).get('S', ''),
                'brand_name': item.get('brand', {}).get('S', ''),
                'sku': item.get('sku', {}).get('S', ''),
                'barcode': item.get('barcode', {}).get('S', ''),
                'selling_price': float(item.get('selling_price', {}).get('N', '0')),
...
async def update_product_in_db(product_id: str, store_id: str, data: dict):
    ...
    try:
        ...
        if 'name' in data or 'product_name' in data:
            ...
            expression_values[':notes'] = {'S': data.get('name') or data.get('product_name')}
        ...
        if 'price' in data or 'selling_price' in data:
            update_expression += ", selling_price = :price"
        ...
        if 'stock_quantity' in data or 'current_stock' in data:
            update_expression += ", current_stock = :stock"
        ...
        # No need for ExpressionAttributeNames since we're using 'notes' which is not a reserved keyword
...
async def get_all_global_products(request: Request):
    ...
    try:
        # Check admin permissions
        user_data = get_user_from_jwt(request)
        if not user_data or (user_data.get('role') != 'admin' and user_data.get('role') != 'super_admin'):
            return {'success': False, 'error': 'Admin access required'}
        ...
        # Helper function to convert DynamoDB Map to Python dict
        def dynamodb_map_to_dict(dynamo_map):
            """Convert DynamoDB Map format to regular Python dict"""
            result = {}
            for key, value in dynamo_map.items():
                if 'S' in value:
                    result[key] = value['S']
                    ...
                    result[key] = [dynamodb_map_to_dict(v['M']) if 'M' in v else v.get('S', '') for v in value['L']]
            return result
        ...
        for item in response.get('Items', []):
            product = {
                ...
                'barcode': item.get('barcode', {}).get('S', ''),
                'regional_names': dynamodb_map_to_dict(item.get('regional_names', {}).get('M', {})),
                'attributes': dynamodb_map_to_dict(item.get('attributes', {}).get('M', {}))
            }
...
async def create_global_product(request: Request):
    ...
    try:
        ...
        item = {
            ...
            'status': {'S': body.get('status', 'pending')},  # active, inactive, pending
...
async def delete_global_product(product_id: str, request: Request):
    ...
    try:
        ...
        # Delete the product from global products table
        dynamodb.delete_item(
            TableName='vyaparai-global-products-prod',
            Key={'product_id': {'S': product_id}}
        )

        logger.info(f"Deleted global product: {product_id}")
        ...
        logger.error(f"Error deleting global product: {e}")
        return {'success': False, 'error': str(e)}
...
async def bulk_import_products(request: Request):
    """Bulk import products for admin seeding (LEGACY - will be deprecated)"""
    try:
        # Check admin permissions
        user_data = get_user_from_jwt(request)
...
# ===== NEW ASYNC IMPORT ENDPOINTS =====

@app.get("/api/v1/admin/products/import/upload-url")
async def get_admin_upload_url(request: Request, filename: str):
    """
    Generate presigned S3 upload URL for admin CSV import
    ...
    try:
        ...
        job_id = f"admin_import_{timestamp}_{unique_id}"
        
        # Construct S3 key
        s3_key = f"admin-imports/{job_id}/input.csv"
        ...
        try:
            ...
            return {'success': False, 'error': f"Failed to generate upload URL: {str(e)}"}
...
@app.post("/api/v1/admin/products/import/process")
async def process_admin_import(request: Request):
    """
    Initiate async processing of uploaded admin CSV
    
    Payload: { "job_id": "admin_import_20250106_abc123" }
    """
    try:
        # Check admin permissions
        user_data = get_user_from_jwt(request)
        ...
        s3_key = f"admin-imports/{job_id}/input.csv"
        if not import_service.verify_s3_object('vyapaarai-bulk-uploads-prod', s3_key):
            return {'success': False, 'error': 'CSV file not found in S3'}
        
        # Quick validation: check file size, extension, basic structure
        try:
            file_info = import_service.get_file_info('vyapaarai-bulk-uploads-prod', s3_key)
            if file_info['size'] > 100 * 1024 * 1024:  # 100 MB limit
                return {'success': False, 'error': 'File too large (max 100MB)'}
        except Exception as e:
            return {'success': False, 'error': f'Failed to get file info: {str(e)}'}
        
        # Parse first few rows to validate CSV structure
        validation_result = import_service.validate_csv_structure(
        ...
        import_options = ImportOptions(
            ...
            notification_email=user_data.get('email')
        )
        ...
        job = import_service.create_job(
            ...
            s3_input_key=s3_key,
            created_by_user_id=user_data.get('user_id', 'admin'),
            created_by_email=user_data.get('email', 'admin@example.com'),
            import_options=import_options,
            estimated_rows=validation_result['estimated_rows'],
            input_filename=f"admin_import_{job_id}.csv",
            input_file_size_bytes=file_info['size']
        )
        ...
        try:
            ...
            response = lambda_client.invoke(
                FunctionName='vyapaarai-process-import-job',
                InvocationType='Event',  # Async invocation
                Payload=json.dumps(payload)
            )

            logger.info(f"Triggered processing Lambda for job {job_id}: {response['StatusCode']}")
        except Exception as e:
            logger.error(f"Error triggering processing: {e}")
            # Continue anyway - the job is created and can be retried
...
@app.get("/api/v1/admin/products/import/{job_id}/status")
async def get_admin_import_status(job_id: str, request: Request):
    """Poll job status with progress details"""
    try:
        # Check admin permissions
        user_data = get_user_from_jwt(request)
        if not user_data or user_data.get('role') not in ['admin', 'super_admin']:
        ...
        if user_data.get('role') != 'super_admin' and job['created_by_user_id'] != user_data.get('user_id'):
        ...
        # Generate presigned URL for error report if exists
        error_report_url = None
        if job.get('s3_error_report_key'):
            s3_client = boto3.client('s3')
            error_report_url = s3_client.generate_presigned_url(
                'get_object',
                Params={
                    'Bucket': job['s3_bucket'],
                    'Key': job['s3_error_report_key']
                },
        ...
        return {
            ...
            "timestamps": {
                ...
                "estimated_completion": job.get('estimated_completion_at')
            },
            "error_report_url": error_report_url,
            "recent_errors": job.get('recent_errors', [])[:10]  # Last 10 errors
        }
...
async def customer_register(request: Request):
    ...
    try:
        ...
        customer_data = {
            ...
            'email': {'S': email},
            'phone': {'S': phone},
            'mobile': {'S': body.get('mobile', phone)},
            'first_name': {'S': first_name},
            'last_name': {'S': last_name},
            'full_name': {'S': f'{first_name} {last_name}'},
            'password_hash': {'S': password_hash.decode('utf-8')},
            'password_algorithm': {'S': 'bcrypt'},
            'role': {'S': 'customer'},
            'status': {'S': 'active'},
            ...
            'created_at': {'S': now},
            'updated_at': {'S': now},
            'registration_source': {'S': 'web'}
        }

        # Add address if provided
        if body.get('address'):
            address = body['address']
            customer_data['address'] = {'M': {
                'line1': {'S': address.get('line1', '')},
                'line2': {'S': address.get('line2', '')},
                'city': {'S': address.get('city', '')},
                'state': {'S': address.get('state', '')},
                'pincode': {'S': address.get('pincode', '')},
                'landmark': {'S': address.get('landmark', '')}
            }}

        # Add Aadhaar if provided
        if body.get('aadhaar_number'):
            customer_data['aadhaar_number'] = {'S': body['aadhaar_number']}

        # Add payment methods if provided
        if body.get('payment_methods'):
            pm = body['payment_methods']
            payment_methods = {}
            if pm.get('upi'):
                payment_methods['upi'] = {'S': pm['upi']}
            if pm.get('paytm'):
                payment_methods['paytm'] = {'S': pm['paytm']}
            if pm.get('cardNumber'):
                # Store only last 4 digits for security
                payment_methods['card_last4'] = {'S': pm['cardNumber'][-4:]}

            if payment_methods:
                customer_data['payment_methods'] = {'M': payment_methods}

        # Add preferences
        if body.get('preferences'):
            prefs = body['preferences']
            customer_data['preferences'] = {'M': {
                'newsletter': {'BOOL': prefs.get('newsletter', True)},
                'notifications': {'BOOL': prefs.get('notifications', True)}
            }}

        # Initialize customer wallet and loyalty
        customer_data['wallet_balance'] = {'N': '0'}
        customer_data['loyalty_points'] = {'N': '0'}
        customer_data['total_orders'] = {'N': '0'}
        customer_data['total_spent'] = {'N': '0'}
        ...
        logger.info(f"[CustomerRegistration] New customer registered: {email}")
...
async def customer_login(request: Request):
    ...
    try:
        ...
        customer_status = customer.get('status', {}).get('S', 'active')
        if customer_status != 'active':
            return {'success': False, 'error': 'Account is inactive'}

        # Verify password with bcrypt
        stored_password = customer.get('password_hash', {}).get('S', '')
        try:
            import bcrypt
            password_valid = bcrypt.checkpw(password.encode('utf-8'), stored_password.encode('utf-8'))
        except:
        ...
        logger.info(f"[CustomerLogin] Customer logged in: {email}")
        ...
        return {
            ...
            'customer': {
                'id': customer_id,
                'email': email,
                'first_name': customer.get('first_name', {}).get('S', ''),
                'last_name': customer.get('last_name', {}).get('S', ''),
                'phone': customer.get('phone', {}).get('S', ''),
                'role': 'customer',
                'wallet_balance': float(customer.get('wallet_balance', {}).get('N', '0')),
                'loyalty_points': int(customer.get('loyalty_points', {}).get('N', '0'))
            }
        }
...
# OTP Storage (in-memory for MVP, should use Redis in production)
otp_storage = {}

@app.post("/api/v1/customer/auth/send-otp")
async def send_customer_otp(request: Request):
    ...
    try:
        ...
        # TODO: Send SMS via AWS SNS or third-party service
        # For now, log OTP (development only)
        logger.info(f"[OTP] Phone: {phone}, OTP: {otp}")

        return {
            'success': True,
            'message': 'OTP sent successfully',
            'otp': otp if os.environ.get('ENV') == 'development' else None  # Only in dev
        }
...
@app.post("/api/v1/customer/auth/verify-otp")
async def verify_customer_otp(request: Request):
    ...
    try:
        ...
        # Check OTP
        stored_otp_data = otp_storage.get(phone)
        ...
        response = local_dynamodb.scan(
            TableName=TABLE_NAMES['users'],
            FilterExpression='phone = :phone AND #role = :customer_role',
            ExpressionAttributeNames={
                '#role': 'role'
            },
            ExpressionAttributeValues={
                ':phone': {'S': phone},
                ':customer_role': {'S': 'customer'}
            }
        )
        ...
        if response['Items']:
            ...
            email = customer.get('email', {}).get('S', '')

            # Update last login
            local_dynamodb.update_item(
                TableName=TABLE_NAMES['users'],
                Key={'id': {'S': customer_id}},
                UpdateExpression='SET last_login = :login_time, phone_verified = :verified',
                ExpressionAttributeValues={
                    ':login_time': {'S': datetime.utcnow().isoformat()},
                    ':verified': {'BOOL': True}
                }
            ...
            customer_id = f'customer_phone_{phone}'
            now = datetime.utcnow().isoformat()
            ...
            customer_data = {
                ...
                'phone': {'S': phone},
                'mobile': {'S': phone},
                'first_name': {'S': 'Customer'},
                'last_name': {'S': phone[-4:]},  # Use last 4 digits
                'full_name': {'S': f'Customer {phone[-4:]}'},
                'role': {'S': 'customer'},
                'status': {'S': 'active'},
                ...
                'created_at': {'S': now},
                'updated_at': {'S': now},
                'registration_source': {'S': 'phone_otp'},
                'wallet_balance': {'N': '0'},
                'loyalty_points': {'N': '0'},
                'total_orders': {'N': '0'},
                'total_spent': {'N': '0'}
            ...
            logger.info(f"[CustomerOTPRegister] New customer via OTP: {phone}")
...
# ============================================================================
# ORDER MANAGEMENT ENDPOINTS (Adapted from KiranaVoiceEnterprise)
# ============================================================================
...
def generate_order_number():
    """Generate unique order number: ORD_YYYYMMDD_XXXXXX"""
    from datetime import datetime
    import random
    date_str = datetime.utcnow().strftime('%Y%m%d')
    random_suffix = str(random.randint(100000, 999999))
    return f'ORD_{date_str}_{random_suffix}'
...
async def create_order(request: Request):
    ...
    try:
        ...
        customer_data = get_user_from_jwt(request)
        if not customer_data or customer_data.get('role') != 'customer':
        ...
        if not store_id or not items:
            return {'success': False, 'error': 'Store ID and items are required'}
        ...
        order_data = {
            ...
            'items': {'L': [
                {'M': {
                    'product_id': {'S': item.get('product_id', '')},
                    'name': {'S': item['name']},
                    'quantity': {'N': str(item['quantity'])},
                    'unit': {'S': item.get('unit', 'piece')},
                    'price': {'N': str(item['price'])},
                    'total': {'N': str(item['quantity'] * item['price'])}
                }} for item in items
            ]},
            'subtotal': {'N': str(subtotal)},
            'delivery_fee': {'N': str(delivery_fee)},
            'tax': {'N': str(tax)},
            'total': {'N': str(total)},
            'payment_status': {'S': PAYMENT_STATUS['COD'] if body.get('payment_method', 'cod') == 'cod' else PAYMENT_STATUS['PENDING']},
            'payment_method': {'S': body.get('payment_method', 'cod')},
            'delivery_address': {'S': body.get('delivery_address', '')},
            'delivery_instructions': {'S': body.get('delivery_instructions', '')},
            'customer_note': {'S': body.get('notes', '')},
            'source': {'S': body.get('source', 'web')},
            'created_at': {'S': now},
            'updated_at': {'S': now}
        }
        ...
        history_data = {
            'id': {'S': f'history_{order_number}_{int(datetime.utcnow().timestamp())}'},
            'order_id': {'S': order_id},
            'order_number': {'S': order_number},
            'from_status': {'S': ''},
            'to_status': {'S': ORDER_STATUS['PLACED']},
            'changed_by': {'S': customer_id},
            'notes': {'S': 'Order placed'},
            'created_at': {'S': now}
        }

        local_dynamodb.put_item(
            TableName='vyaparai-order-history-prod',
            Item=history_data
        )

        logger.info(f"[OrderCreated] Order: {order_number}, Customer: {customer_id}, Total: {total}")
...
@app.get("/api/v1/orders/customer")
async def get_customer_orders(request: Request):
    ...
    try:
        ...
        # Query orders by customer (requires GSI)
        response = local_dynamodb.scan(
            TableName='vyaparai-orders-prod',
        ...
        for item in response.get('Items', []):
            orders.append({
                ...
                'payment_status': item['payment_status']['S'],
                'payment_method': item.get('payment_method', {}).get('S', 'cod'),
                'created_at': item['created_at']['S'],
                'items_count': len(item.get('items', {}).get('L', []))
            })
        ...
        logger.error(f"Get customer orders error: {str(e)}")
        return {'success': False, 'error': 'Failed to get orders'}


@app.get("/api/v1/orders/{order_number}")
async def get_order_details(request: Request, order_number: str):
    ...
    try:
        ...
        response = local_dynamodb.scan(
            TableName='vyaparai-orders-prod',
            FilterExpression='order_number = :order_number',
            ExpressionAttributeValues={
                ':order_number': {'S': order_number}
            }
        )
        ...
        # Verify customer owns this order (or is store owner/admin)
        if (customer_data.get('role') == 'customer' and
            order['customer_id']['S'] != customer_data.get('user_id')):
            return {'success': False, 'error': 'Unauthorized'}

        # Parse order data
        items = []
        for item in order.get('items', {}).get('L', []):
            item_data = item['M']
            items.append({
                'product_id': item_data.get('product_id', {}).get('S', ''),
                'name': item_data['name']['S'],
                'quantity': int(item_data['quantity']['N']),
                'unit': item_data.get('unit', {}).get('S', 'piece'),
                'price': float(item_data['price']['N']),
                'total': float(item_data['total']['N'])
            })
        ...
        return {
            ...
            'order': {
                ...
                'customer_id': order['customer_id']['S'],
                'store_id': order['store_id']['S'],
                'status': order['status']['S'],
                'items': items,
                'subtotal': float(order['subtotal']['N']),
                'delivery_fee': float(order.get('delivery_fee', {}).get('N', '0')),
                'tax': float(order.get('tax', {}).get('N', '0')),
                'total': float(order['total']['N']),
                'payment_status': order['payment_status']['S'],
                'payment_method': order.get('payment_method', {}).get('S', 'cod'),
                'delivery_address': order.get('delivery_address', {}).get('S', ''),
                'delivery_instructions': order.get('delivery_instructions', {}).get('S', ''),
                'customer_note': order.get('customer_note', {}).get('S', ''),
                'created_at': order['created_at']['S'],
                'updated_at': order['updated_at']['S']
            }
        }
...
@app.get("/api/v1/orders/{order_number}/tracking")
async def get_order_tracking(request: Request, order_number: str):
    """Get order status tracking timeline"""
    try:
        # Get DynamoDB client
        local_dynamodb = get_dynamodb_client()
        if not local_dynamodb:
        ...
        for item in response.get('Items', []):
            timeline.append({
                'from_status': item.get('from_status', {}).get('S', ''),
                'to_status': item['to_status']['S'],
                'changed_by': item.get('changed_by', {}).get('S', ''),
        ...
        logger.error(f"Get order tracking error: {str(e)}")
        return {'success': False, 'error': 'Failed to get tracking'}


@app.patch("/api/v1/orders/{order_number}/status")
async def update_order_status(request: Request, order_number: str):
    """Update order status (store owner/admin only)"""
    try:
        body = await request.json()

        # Get user from JWT
        user_data = get_user_from_jwt(request)
        if not user_data or user_data.get('role') not in ['store_owner', 'super_admin']:
            return {'success': False, 'error': 'Store owner or admin authentication required'}
        ...
        logger.info(f"[OrderStatusUpdated] Order: {order_number}, {old_status} → {new_status}, By: {user_id}")
...
@app.post("/api/v1/orders/{order_number}/cancel")
async def cancel_order(request: Request, order_number: str):
    """Cancel an order (customer or store owner)"""
    try:
        body = await request.json()
        ...
        order_id = order['id']['S']
        customer_id = order['customer_id']['S']
        current_status = order['status']['S']

        # Check permissions
        if user_role == 'customer' and customer_id != user_id:
            return {'success': False, 'error': 'Unauthorized'}
        ...
        if current_status == ORDER_STATUS['CANCELLED']:
        ...
        local_dynamodb.update_item(
            TableName='vyaparai-orders-prod',
            Key={'id': {'S': order_id}},
            UpdateExpression='SET #status = :cancelled, cancellation_reason = :reason, updated_at = :updated_at',
            ExpressionAttributeNames={'#status': 'status'},
            ExpressionAttributeValues={
                ':cancelled': {'S': ORDER_STATUS['CANCELLED']},
                ':reason': {'S': reason},
                ':updated_at': {'S': now}
            }
        )
        ...
        history_data = {
            ...
            'order_id': {'S': order_id},
            'order_number': {'S': order_number},
            'from_status': {'S': current_status},
            'to_status': {'S': ORDER_STATUS['CANCELLED']},
            'changed_by': {'S': user_id},
            'notes': {'S': f'Order cancelled. Reason: {reason}'},
            'created_at': {'S': now}
        }
        ...
        logger.info(f"[OrderCancelled] Order: {order_number}, By: {user_id}, Reason: {reason}")
...
@app.get("/api/v1/orders/store/{store_id}")
async def get_store_orders(request: Request, store_id: str):
    """Get all orders for a store (store owner only)"""
    try:
        # Get user from JWT
        user_data = get_user_from_jwt(request)
        if not user_data or user_data.get('role') not in ['store_owner', 'super_admin']:
        ...
        logger.error(f"Get store orders error: {str(e)}")
        return {'success': False, 'error': 'Failed to get orders'}
...
async def get_cart(request: Request):
    ...
    try:
        ...
        try:
            ...
            items = []
            subtotal = 0
            for item in cart.get('items', {}).get('L', []):
                item_data = item['M']
                item_total = float(item_data['price']['N']) * int(item_data['quantity']['N'])
                items.append({
                    'product_id': item_data['product_id']['S'],
                    'store_id': item_data['store_id']['S'],
                    'name': item_data['name']['S'],
                    'price': float(item_data['price']['N']),
                    'quantity': int(item_data['quantity']['N']),
                    'unit': item_data.get('unit', {}).get('S', 'piece'),
                    'image': item_data.get('image', {}).get('S', ''),
                    'total': item_total
                })
                subtotal += item_total
            ...
            return {
                ...
                'cart': {
                    ...
                    'subtotal': subtotal,
                    'updated_at': cart.get('updated_at', {}).get('S', '')
                }
            }
...
async def add_to_cart(request: Request):
    ...
    try:
        ...
        if not all([product_id, store_id, name, price]):
        ...
        # Get existing cart
        cart_response = local_dynamodb.get_item(
            TableName='vyaparai-carts-prod',
            Key={'customer_id': {'S': customer_id}}
        )
        ...
        if 'Item' in cart_response:
            existing_items = cart_response['Item'].get('items', {}).get('L', [])

        # Check if item already exists in cart
        item_found = False
        for idx, item in enumerate(existing_items):
            if item['M']['product_id']['S'] == product_id:
                # Update quantity
                current_qty = int(item['M']['quantity']['N'])
                existing_items[idx]['M']['quantity']['N'] = str(current_qty + quantity)
                item_found = True
                break
        ...
        if not item_found:
            new_item = {
                'M': {
                    ...
                    'unit': {'S': body.get('unit', 'piece')},
                    'image': {'S': body.get('image', '')},
                    'added_at': {'S': now}
                }
            }
        ...
        local_dynamodb.put_item(
            ...
            Item={
                'customer_id': {'S': customer_id},
                'items': {'L': existing_items},
                'updated_at': {'S': now},
                'ttl': {'N': str(int(datetime.utcnow().timestamp()) + 30 * 24 * 60 * 60)}  # 30 days TTL
            }
        )

        logger.info(f"[CartItemAdded] Customer: {customer_id}, Product: {product_id}, Qty: {quantity}")
...
async def update_cart_item(request: Request, product_id: str):
    ...
    try:
        ...
        for item in existing_items:
            if item['M']['product_id']['S'] == product_id:
                item_found = True
                if quantity > 0:
                    item['M']['quantity']['N'] = str(quantity)
                    updated_items.append(item)
                # If quantity is 0, don't add to updated_items (remove it)
            else:
                updated_items.append(item)
        ...
        local_dynamodb.put_item(
            ...
            Item={
                'customer_id': {'S': customer_id},
                'items': {'L': updated_items},
                'updated_at': {'S': now},
                'ttl': {'N': str(int(datetime.utcnow().timestamp()) + 30 * 24 * 60 * 60)}
            }
        )

        logger.info(f"[CartItemUpdated] Customer: {customer_id}, Product: {product_id}, Qty: {quantity}")
...
async def remove_from_cart(request: Request, product_id: str):
    ...
    try:
        ...
        # Remove item
        updated_items = [item for item in existing_items if item['M']['product_id']['S'] != product_id]
        ...
        logger.info(f"[CartItemRemoved] Customer: {customer_id}, Product: {product_id}")
...
async def clear_cart(request: Request):
    ...
    try:
        ...
        logger.info(f"[CartCleared] Customer: {customer_id}")
...
async def get_nearby_stores(
    ...
    """
    Get nearby stores with enterprise-grade performance optimizations
    
    Performance Features:
    - Pagination support (limit parameter)
    - Database indexing optimization
    - Caching for geocoded addresses
    ...
    try:
        ...
        # Since city and state are embedded in address JSON, use scan with filters
        scan_params = {
            'TableName': TABLE_NAMES['stores'],
            'Limit': limit
        }
        
        # Add filter expressions for city/state if provided
        filter_expressions = []
        expression_values = {}
        expression_names = {}
        
        if city:
            filter_expressions.append('contains(address, :city)')
            expression_values[':city'] = {'S': city}
        
        if state:
            filter_expressions.append('contains(address, :state)')
            expression_values[':state'] = {'S': state}
        ...
        stores = []
        for item in response.get('Items', []):
            # Parse address - it's stored as a JSON string, not a DynamoDB Map
            address_data = {}
            try:
                if 'address' in item and 'S' in item['address']:
                    address_data = json.loads(item['address']['S'])
            ...
            store = {
                ...
                'address': {
                    ...
                    'full': f"{address_data.get('street', '')}, {address_data.get('city', '')}, {address_data.get('state', '')} {address_data.get('pincode', '')}".strip(', ')
                },
                'category': item.get('category', {}).get('S', 'General Store'),
                'isOpen': item.get('isOpen', {}).get('BOOL', True),
                'rating': float(item.get('rating', {}).get('N', '4.5')),
                'openingHours': item.get('openingHours', {}).get('S', '9:00 AM - 9:00 PM'),
        ...
        return {
            ...
            "performance_notes": {
                "pagination": f"Limited to {limit} results",
                "indexing": "Using GSI for efficient querying",
                "caching": "Geocoded addresses cached for 24h"
            }
        }
