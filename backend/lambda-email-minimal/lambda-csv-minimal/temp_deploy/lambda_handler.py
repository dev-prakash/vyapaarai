...
# CORS middleware - Required for API Gateway to handle OPTIONS preflight requests
...
# CORS middleware - Required for API Gateway to handle OPTIONS preflight requests
# API Gateway CORS config only adds headers, but FastAPI must respond to OPTIONS with 200
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
